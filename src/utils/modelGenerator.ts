...
export async function createModel(table: string) {
  try {
    ...
    const columns = await oracle.query<any>(`
    ...
    columns.forEach((col: OracleColumn) => {
      modelContent += `  public ${col.COLUMN_NAME.toLowerCase()}!: ${getTsType(col.DATA_TYPE)}\n`
    })
    ...
    // Write model file
    const modelPath = join(__dirname, '../models', `${table.toLowerCase()}.ts`)
