...
- Development: เก็บ “SQL Statement” เป็นไฟล์ชื่อ `<APP_ID>_<SQL_NO>.sql` (แนะนำ pad SQL_NO 3 หลัก เช่น `0_001.sql`) เพื่อให้ `getSqlStmt()` อ่านไปใช้งานได้ทันทีโดยไม่ต้องรอให้ user อัปเดต DB ก่อน — ไฟล์อยู่โฟลเดอร์นี้ (`server/sqltabs`) หรือ override ด้วย `SQLTAB_DIR`
